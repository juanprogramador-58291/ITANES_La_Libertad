...
public class PlaceDataSeeder {
    ...
    public PlaceDataSeeder(Context context) {
    ...
    public void seedInitialData() {
        executorService.execute(() -> {
            try {
                int count = placeRepository.getPlacesCount();
                if (count == 0) {
                    Log.d(TAG, "La tabla places esta vacia. Procediendo a insertar los 5 lugares iniciales...");
                    ...
                    placeRepository.insertPlaces(initialPlaces);
                    int finalCount = placeRepository.getPlacesCount();
                    Log.i(TAG, "PlaceDataSeeder: " + finalCount + " lugares disponibles en Room");
                } else {
                    int finalCount = placeRepository.getPlacesCount();
                    Log.i(TAG, "PlaceDataSeeder: " + finalCount + " lugares disponibles en Room");
                }
            } catch (Exception e) {
                Log.e(TAG, "Error durante la inserción de datos iniciales en la base de datos", e);
            }
        });
    }
}
